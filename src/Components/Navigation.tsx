...
export default function Navigation() {
 ...
 const  isActive = (path:string)=>{
  ...
  return (
    <>
    <nav className='w-full bg-secondary h-20 flex justify-between sticky top-0 right-0 z-10'>
    ...
    {isClicked && (
        <ul
          className={
            isClicked === true
              ? `lg:hidden  navigation h-screen  w-full transform transition duration-75  flex flex-col gap-10 items-center bg-secondary shadow-sm animate-open-nav z-20 fixed  mt-0`
        ...
        >
          ...
          >
            <Link href={`/`} onClick={() => setClicked(false)} className='text-primary hover:text-text-primary justify-center items-center'> Home</Link>
          </li>
          {
        links.map(({id,link})=>(
           <li
              key={id}
              className={isActive(link)? "active px-4 flex justify-center items-center gap-2 cursor-pointer capitalize py-6  w-40 text-primary text-2xl" :"px-4 flex justify-center items-center gap-2 cursor-pointer capitalize py-6  w-40 text-text-primary hover:text-text-primary hover:bg-primary hover:shadow-md hover:rounde hover:text-2xl"}
            >
              <Link href={`/Bhendogu/${link}`} className={ "text-md h-full w-full justify-center items-center"}>
                {link}
              </Link>
            </li>
        ))
      }
          <li className={isActive('/')? "activeQute px-4 flex justify-center items-center gap-2 cursor-pointer capitalize py-6  w-40 text-primary text-2xl" :"btn-primary bg-text-primary w-44 text-blue-600 text-center flex justify-center gap-5 items-center h-20"}>
          <Link
            href="/Bhendogu/getAquote"
            className="btn-primary bg-text-primary w-44 text-blue-600 text-center flex justify-center gap-5 items-center h-20"
            onClick={() => setClicked(false)}
          >
            <FaMessage className="mr-2 text-2xl text-primary" /> Get A Quote
           </Link>
